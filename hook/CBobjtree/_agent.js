...
// src/agent.ts
var Agent = class {
  moduleBase = Process.enumerateModules()[0].base;
  cachedObjCResolver = null;
  cachedModuleResovler = null;
  objc_msgSend = null;
  stackDepth = 0;
  installedHooks = 0;
  pendingEvents = [];
  ...
  installObjCHook(pattern) {
    for (const m2 of this.getObjCResolver().enumerateMatches(pattern)) {
      this.installHook(m2.address, m2.name);
    }
  }
  getObjCResolver() {
  ...
  installFunctionHook(pattern) {
    ...
    for (const m2 of this.getModuleResolver().enumerateMatches(`exports:${q.module}!${q.function}`)) {
      this.installHook(m2.address, m2.name);
    }
  }
  getModuleResolver() {
  ...
  installHook(pointer, funcDescription) {
    const objc_msgSend2 = this.objc_msgSend;
    const agent2 = this;
    Interceptor.attach(pointer, {
      onEnter: function(args) {
        const originThreadId = this.threadId;
        agent2.emit([0, funcDescription]);
        this.hook = Interceptor.attach(objc_msgSend2, {
          onEnter: function(args2) {
            if (this.threadId == originThreadId) {
              agent2.objcOnEnter(this, args2);
            }
          }
        });
      },
  ...
  objcOnEnter(ctx, args) {
    ...
    const id = args[0];
    const selector2 = args[1].readCString();
    let cls;
    let typeQualifier;
    if (frida_objc_bridge_default.api.object_isClass(id)) {
      typeQualifier = "+";
      cls = id;
    } else {
      typeQualifier = "-";
      cls = frida_objc_bridge_default.api.object_getClass(id);
    }
    let clsName = frida_objc_bridge_default.api.class_getName(cls).readCString();
    let objcMessage = `${typeQualifier}[${clsName} ${selector2}]`;
...
function parseModuleFunctionPattern(pattern) {
  ...
  if (tokens.length === 1) {
    ...
    m2 = tokens[0] === "" ? "*" : tokens[0];
    f2 = tokens[1] === "" ? "*" : tokens[1];
  }
  return {
    module: m2,
    function: f2
