...
class BuilderAction(argparse.Action):
    def __init__(self, option_strings, dest, nargs=None, **kwargs):
        self.handler = kwargs.pop("handler")  # 传入的 pb.hook_xxx
        super().__init__(option_strings, dest, nargs=nargs, **kwargs)

    def __call__(self, parser, namespace, values, option_string=None):
        self.handler(values)                  # 触发你的逻辑
        setattr(namespace, self.dest, values) # 仍把值写回 args，方便后续使用
...
class TestApplication(ConsoleApplication, UI):
    ...
    def _add_options(self, parser):
        ...
        parser.add_argument(
            ...
            handler=pb.hook_objc_method,
        )

        parser.add_argument(
            "-i",
            metavar="FUNCTION",
            help="include FUNCTION",
            action=BuilderAction,
            handler=pb.hook_function,
        )

        parser.add_argument(
            "-a",
            metavar="FUNCTION_OFFSET",
            help="add FUNCTION_OFFSET, relative to binary base",
            type=lambda x: int(x, 0),  # 支持 16 / 0x10
            action=BuilderAction,
            handler=pb.hook_function_offset,
        )

        parser.add_argument(
            "-L",
            metavar="STACK_DEPTH",
            help="trace functions up to STACK_DEPTH, default is 8",
            type=int,
            action=BuilderAction,
            handler=pb.set_stack_depth,
        )
