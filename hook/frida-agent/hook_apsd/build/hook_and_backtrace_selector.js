...
// src/hook_and_backtrace_selector.ts
function printBacktrace(ctx, fuzzy = true, max = 32) {
  const mode = fuzzy ? Backtracer.FUZZY : Backtracer.ACCURATE;
  const addrs = Thread.backtrace(ctx, mode);
  console.log(`
=== Backtrace (${fuzzy ? "FUZZY" : "ACCURATE"}) ===`);
  for (let i = 0; i < Math.min(addrs.length, max); i++) {
    const sym = DebugSymbol.fromAddress(addrs[i]);
...
if (!frida_objc_bridge_default.available) {
  ...
  const Cls = frida_objc_bridge_default.classes[CLASS_NAME];
  if (!Cls) {
...
[*] Enter [${CLASS_NAME} ${SELECTOR}]`);
        try {
          console.log("self =", new frida_objc_bridge_default.Object(args[0]).toString());
        } catch (_) {
        }
        try {
          console.log("arg2 =", new frida_objc_bridge_default.Object(args[2]).toString());
        } catch (_) {
        }
