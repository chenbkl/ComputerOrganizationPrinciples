...
function inWindow(tid) { return (depthByTid.get(tid) ?? 0) > 0; }
function enterWindow(tid) { depthByTid.set(tid, (depthByTid.get(tid) ?? 0) + 1); }
function leaveWindow(tid) {
...
/* =============== 小工具 =============== */
function callerSym(ret) {
    try {
        const ds = DebugSymbol.fromAddress(ret);
        return `${ds.moduleName || ''}!${ds.name || ret}`;
    }
    catch {
        return ret.toString();
    }
}
function shallowValSummary(p) {
    try {
...
function withAutoreleasePool(fn) {
    try {
...
setImmediate(() => {
    ...
    if (!M) {
        console.log(`Parent not found: [${PARENT_CLASS} ${PARENT_SEL}]`);
        return;
    }
    console.log(`[hook] parent [${PARENT_CLASS} ${PARENT_SEL}] types=${M.types}`);
    ...
    if (OPTIONS.HOOK_CF)
        hookCFDict();
    // —— 下游（可选）：字典读取（按模块过滤） ——
    if (OPTIONS.HOOK_READS)
        hookDictReadsFiltered(OPTIONS.FILTER_READS_BY_MODULE);
...
function hookNSMutableDictWrites() {
    ...
    function onSet(args) {
        ...
        try {
            ...
            const sv = shallowValSummary(args[2]);
    ...
    if (implA)
        Interceptor.attach(implA, { onEnter: onSet });
    if (implB)
        Interceptor.attach(implB, { onEnter: onSet });
    // 标注 __NSDictionaryM 的 init → class
    const initImp = MDM['- init']?.implementation;
    if (initImp)
        Interceptor.attach(initImp, {
            onLeave(rv) {
                const tid = this.threadId;
                if (!inWindow(tid))
                    return;
                try {
                    eventsByTid.get(tid)?.push({ kind: 'class', dict: rv, cls: '__NSDictionaryM' });
                }
                catch { }
            }
        });
...
function hookCFDict() {
    let cf;
    try {
        cf = Process.getModuleByName('CoreFoundation');
    }
    catch {
        return;
    }
    ...
    if (CFDictionaryCreateMutable)
        Interceptor.attach(CFDictionaryCreateMutable, {
            onLeave(rv) {
                const tid = this.threadId;
                if (!inWindow(tid))
                    return;
    ...
    if (CFDictionarySetValue)
        Interceptor.attach(CFDictionarySetValue, {
            onEnter(args) {
                const tid = this.threadId;
                if (!inWindow(tid))
                    return;
...
function hookDictReadsFiltered(moduleContains) {
    const I = ObjC.classes.__NSDictionaryI;
    const M = ObjC.classes.__NSDictionaryM;
    ObjC.Object;
    const specs = [];
    if (I && I['- objectForKey:'])
        specs.push([I, '- objectForKey:']);
    if (I && I['- valueForKey:'])
        specs.push([I, '- valueForKey:']);
    if (M && M['- objectForKey:'])
        specs.push([M, '- objectForKey:']);
    if (M && M['- valueForKey:'])
        specs.push([M, '- valueForKey:']);
    for (const [Cls, sel] of specs) {
        Interceptor.attach(Cls[sel].implementation, {
            onEnter(args) {
                ...
                if (!mod.includes(moduleContains))
                    return;
                let key = '<non-str>';
                try {
                    key = new ObjC.Object(args[2]).toString();
                }
                catch { }
...
/* =============== 漂亮打印 =============== */
function prettyDescribeNSObject(o) {
    ...
    function tryJSON(obj) {
        if (!OPTIONS.JSON_PRETTY)
            return null;
        try {
            if (NSJSON['+ isValidJSONObject:'] && !NSJSON['+ isValidJSONObject:'](obj))
                return null;
            const opts = 1; // pretty
            ...
            if (OPTIONS.JSON_SPACES !== 2)
                return txt; // NSJSON 的 pretty 是固定两个空格，仅原样返回
            return txt;
        }
        catch {
            return null;
    ...
    function describeNSData(o) {
        try {
            ...
            const bytes = o['- bytes']();
            const n = Math.min(64, len | 0), hex = [];
            for (let i = 0; i < n; i++)
                hex.push(('0' + bytes.add(i).readU8().toString(16)).slice(-2));
            return `<NSData len=${len} hex=${hex.join(' ')}${len > n ? ' …' : ''}>`;
        }
        catch {
            return `<NSData ${o.toString()}>`;
        }
    }
    if (o == null)
        return '<nil>';
    ...
    if (o.isKindOfClass_(ObjC.classes.NSData))
        return describeNSData(o);
