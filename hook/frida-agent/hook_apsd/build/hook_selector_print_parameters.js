...
function leavePrinting(tid) {
    const d = (printingDepth.get(tid) ?? 1) - 1;
    if (d <= 0)
        printingDepth.delete(tid);
    else
        printingDepth.set(tid, d);
}
...
setImmediate(() => {
    ...
    const M = C?.[SEL];
    if (!M) {
        console.log(`Method not found: [${CLASS} ${SEL}]`);
        return;
    }
    const retT = M.returnType;
    const argT = M.argumentTypes;
...
/* ======== 入参的“浅打印” ======== */
function safeShallowFormat(t, p, selToString) {
    try {
        switch (t) {
            case 'void': return '(void)';
            case 'SEL': return selToString(p);
            case 'char *': return p.isNull() ? '<nil>' : (p.readUtf8String?.() ?? p.readCString?.() ?? '<empty>');
...
/* ======== 返回值等需要时才做的“完整打印” ======== */
function formatValue(t, p, selToString, pretty) {
    try {
        switch (t) {
            case 'void': return '(void)';
            case 'SEL': return selToString(p);
            ...
            default:
                try {
                    return pretty.describeNSObject(new ObjC.Object(p));
                }
                catch {
                    return p.toString();
                }
...
function makeSelToString() {
    ...
    return (sel) => {
        ...
        if (sel_getName) {
            const c = sel_getName(sel);
            return c.isNull() ? '<nil>' : (c.readUtf8String?.() ?? c.readCString?.() ?? '<empty>');
        }
        return '<unknown_sel>';
    };
}
...
function makePrettyPrinters() {
    ...
    function describeNSData(o) {
        try {
            ...
            if (txt && txt.length)
                return `<NSData len=${len} utf8=${JSON.stringify(txt.slice(0, 200))}${txt.length > 200 ? '…' : ''}>`;
            const bytes = o['- bytes']();
            const n = Math.min(32, len | 0), hex = [];
            for (let i = 0; i < n; i++)
