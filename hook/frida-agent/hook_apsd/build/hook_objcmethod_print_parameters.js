📦
99180 /src/hook_objcmethod_print_parameters.js
62094 /src/hook_objcmethod_print_parameters.js.map
✄
...
var class_name = "APSTCPStream";
var selector2 = "- _connectToServerWithConfiguration:";
if (!frida_objc_bridge_default.available) {
  console.log("[-] ObjC runtime \u4E0D\u53EF\u7528\uFF0C\u672C\u811A\u672C\u9762\u5411 iOS/macOS\u3002");
  throw new Error("ObjC runtime \u4E0D\u53EF\u7528");
}
var Cls = frida_objc_bridge_default.classes[class_name];
if (!Cls) {
...
if (!Cls[selector2]) {
  console.log(`[-] \u672A\u627E\u5230\u65B9\u6CD5\uFF1A[${class_name} ${selector2}]`);
  console.log("[*] \u53EF\u7528\u65B9\u6CD5\u793A\u4F8B\uFF1A", Cls.$ownMethods.slice(0, 10), "...");
  throw new Error(`\u672A\u627E\u5230\u65B9\u6CD5 ${selector2} \u5728\u7C7B ${class_name}`);
}
console.log("\u5DF2\u627E\u5230\u8BE5\u65B9\u6CD5\uFF1A", selector2);
Interceptor.attach(Cls[selector2].implementation, {
  onEnter(args) {
    console.log(`
[*] Enter [${class_name} ${selector2}]`);
    const m2 = frida_objc_bridge_default.classes[class_name][selector2];
    const argument_count = m2.argumentTypes.length;
    if (argument_count <= 2) {
      console.log("\u8BE5\u65B9\u6CD5\u65E0\u53C2\u6570");
    } else {
  ...
  onLeave(retval) {
    try {
      const retObj = new frida_objc_bridge_default.Object(retval);
      console.log("\u8FD4\u56DE\u503C\u7C7B\u578B\u4E3A =", retObj.$className, " \u5730\u5740\u4E3A =", retval);
      dump_print_ObjCobject(retObj);
      console.log(`[*] Leave [${class_name} ${selector2}]`);
    } catch (e) {
...
function dump_print_ObjCobject(obj) {
  if (obj.isKindOfClass_(frida_objc_bridge_default.classes.NSError)) {
    console.log("  \u9519\u8BEF\u4FE1\u606F:", obj.localizedDescription().toString());
  }
  if (obj.isKindOfClass_(frida_objc_bridge_default.classes.NSData)) {
    console.log("  \u6570\u636E\u957F\u5EA6:", obj.length);
  }
  if (obj.isKindOfClass_(frida_objc_bridge_default.classes.NSString)) {
    console.log("  \u5B57\u7B26\u4E32\u5185\u5BB9:", obj.toString());
  }
  if (obj.isKindOfClass_(frida_objc_bridge_default.classes.NSArray)) {
    console.log("  \u6570\u7EC4\u957F\u5EA6:", obj.count());
    console.log("  \u6570\u7EC4\u5185\u5BB9:", obj.toString());
  }
  if (obj.isKindOfClass_(frida_objc_bridge_default.classes.NSDictionary)) {
    console.log("  \u5B57\u5178\u952E\u503C\u5BF9:", obj.toString());
  }
  console.log("\u8BE5\u53C2\u6570\u4E0D\u5C5E\u4E8E\u4E0A\u8FF0\u7C7B\u578B\uFF0C\u76F4\u63A5\u6253\u5370\u5BF9\u8C61\uFF1A", obj.toString());
  dump_objcobject_ivars(obj);
}
function isNSDictionary(o) {
  const n = o.$className;
  return n.includes("Dictionary");
}
function isNSArray(o) {
  const n = o.$className;
  return n.includes("Array");
}
function isFoundation(o) {
  const n = o.$className;
  return n.startsWith("NS") || n.startsWith("__NS") || n === "NSTaggedPointerString";
}
function prettyFoundationScalar(o) {
  const n = o.$className;
  try {
    if (n.includes("String"))
      return `"${o.toString()}"`;
    if (n.includes("Number") || n.includes("Boolean"))
      return o.toString();
    if (n.includes("Data"))
      return `<Data ${o.length()} bytes>`;
    if (n.includes("Date"))
      return o.toString();
  } catch {
  }
...
function dumpNSDictionary(dict, depth = 0, seen = /* @__PURE__ */ new Set()) {
  ...
  try {
    ...
    for (let i = 0; i < count; i++) {
      const kObj = new frida_objc_bridge_default.Object(keyArr.objectAtIndex_(i));
      const kStr = kObj.toString();
      const vPtr = dict.objectForKey_(kObj);
      let line = `${indent}  "${kStr}": `;
      try {
        const vObj = new frida_objc_bridge_default.Object(vPtr);
...
function dumpNSArray(arr, depth = 0, seen = /* @__PURE__ */ new Set()) {
  const indent = "  ".repeat(depth);
  try {
    const n = arr.count();
...
function dump_objcobject_ivars(obj) {
  ...
  try {
    ...
    for (const ivarName of names) {
      try {
        ...
        let vObj = null;
        try {
          vObj = new frida_objc_bridge_default.Object(raw);
        } catch {
          vObj = null;
        }
        if (vObj) {
