...
// src/hook_objcmethod_print_parameters_parseObjcTypes.ts
function parseObjCTypes(types2) {
  let i = 0;
  function skipDigits() {
    while (i < types2.length && types2[i] >= "0" && types2[i] <= "9")
      i++;
  }
  function parseOne() {
    skipDigits();
    ...
    switch (c) {
      ...
      case "V":
        const t = parseOne();
        if (!t)
          return { enc: c, friendly: qualifierFriendly(c) };
      ...
      case "c":
        return { enc: "c", friendly: "char" };
      case "C":
        return { enc: "C", friendly: "unsigned char" };
      case "s":
        return { enc: "s", friendly: "short" };
      case "S":
        return { enc: "S", friendly: "unsigned short" };
      case "i":
        return { enc: "i", friendly: "int" };
      case "I":
        return { enc: "I", friendly: "unsigned int" };
      case "l":
        return { enc: "l", friendly: "long" };
      case "L":
        return { enc: "L", friendly: "unsigned long" };
      case "q":
        return { enc: "q", friendly: "long long" };
      case "Q":
        return { enc: "Q", friendly: "unsigned long long" };
      case "f":
        return { enc: "f", friendly: "float" };
      case "d":
        return { enc: "d", friendly: "double" };
      case "B":
        return { enc: "B", friendly: "bool" };
      case "v":
        return { enc: "v", friendly: "void" };
      case "*":
        return { enc: "*", friendly: "char *" };
      // C-string
      case "{": {
        const end = findMatching(types2, start, "{", "}");
        const enc = types2.slice(start, end + 1);
        i = end + 1;
  ...
  function pointerFriendly(inner) {
    ...
    if (map[inner.enc])
      return map[inner.enc];
    if (inner.enc.startsWith("{"))
      return "struct *";
    if (inner.enc.startsWith("^"))
      return "pointer *";
  ...
  return {
    ...
    args
    // 注意：这里的 args 包含 self(@) 和 _cmd(:) 两个隐含参数
  };
}
...
[*] Enter [${class_name} ${selector2}]`);
    ...
    const sig = parseObjCTypes(m2.types);
    console.log("\u5165\u53C2\u4E3A\uFF1A", frida_objc_bridge_default.selectorAsString(args[1]));
    printArgs(args, sig);
  },
  onLeave(retval) {
    try {
...
[${label}] enc=${enc}, friendly=${friendly}`);
    try {
      if (enc === "@") {
        if (!args[idx].isNull()) {
          const obj = new frida_objc_bridge_default.Object(args[idx]);
        ...
        console.log("  SEL:", frida_objc_bridge_default.selectorAsString(args[idx]));
      } else if (enc === "#") {
        const cls = new frida_objc_bridge_default.Object(args[idx]);
        console.log("  Class:", cls.$className);
      } else if (enc === "c" || enc === "C") {
        console.log("  char =", args[idx].toInt32());
      } else if (enc === "i" || enc === "I" || enc === "s" || enc === "S" || enc === "q" || enc === "Q" || enc === "l" || enc === "L") {
