...
setImmediate(function () {
    ...
    const winByTid = new Map(); // tid -> 窗口栈（支持嵌套）
    ...
    Interceptor.attach(parentImp, {
        onEnter() {
            ...
            const state = { stack: [], roots: [], msgDepth: 0, nodeCount: 0 };
            const root = { sig: `${PARENT_CLASS} ${PARENT_SEL}`, children: [] };
    ...
    function hookMsgSend(name, isSuper) {
        ...
        Interceptor.attach(addr, {
            onEnter(args) {
                ...
                st.msgDepth++;
                // 仅在限制深度内记录
                if (st.msgDepth > MAX_DEPTH)
                    return;
                if (MAX_NODES_PER_WINDOW && st.nodeCount >= MAX_NODES_PER_WINDOW)
                    return;
                // 解析 selector
                let sel = '<sel?>';
                try {
                    sel = selToString(args[1]);
                }
                catch { }
                if (isNoisy(sel))
                    return;
